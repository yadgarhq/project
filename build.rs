...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tonic_prost_build::configure()
        // BOTH: this service SERVES ProjectService and is a CLIENT of
        // ProjectDbService. The twin split means the logic tier always needs
        // the client half — and here both halves are declared in ONE file, so
        // one entry in the compile list below produces both.
        .build_server(true)
        .build_client(true)
        // Both files: prost generates only for the files it is given, and
        // project.proto merely IMPORTING common.proto does not produce a module
        // for it.
        .compile_protos(
            &[
