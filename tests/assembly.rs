...
//! can make: **a `project` configured this way reads exactly these files, so
...
//! case below goes through [`yadgar_project::rotate::watch_set`] — the SAME
...
/// A serving certificate and its key, the CA bundle `project-db` is verified
...
fn generation(san: &str) -> Generation {
    ...
    ca_params
        .distinguished_name
        .push(DnType::CommonName, "yadgar-project assembly test authority");
...
impl Mount {
    fn new(files: &Generation) -> Self {
        let root = std::env::temp_dir().join(format!("yadgar-project-assembly-{}", unique()));
...
/// How `project-db` is verified, and who this service says it is on that hop.
...
fn the_watch_set_holds_every_file_this_deployment_configured() {
    ...
    assert_eq!(
        ...
        "a fully configured `project` reads six files at boot: the listener's leaf and its \
         key, the bundle `project-db` is verified against, the client identity presented on \
...
/// THE GAUGE THIS PROCESS PUBLISHES SAYS `service = "project"`.
///
...
fn the_gauge_names_this_service_and_each_certificate_it_holds() {
    ...
    assert_eq!(
        ...
        vec![
            (
                vec![
                    ("service".to_string(), "project".to_string()),
...
/// identity whose loss stops this service reaching `project-db` (ADR-0516).
///
...
fn the_unreadable_gauge_carries_this_service_and_is_published_at_zero_too() {
    ...
    for (emitted, expected) in [(at_zero, 0.0_f64), (at_one, 1.0_f64)] {
        ...
        assert_eq!(
            labels,
            vec![("service".to_string(), "project".to_string())],
