...
//! stands up this service's own listener through [`yadgar_project::serve::builder`]
...
async fn request(mut channel: Channel) -> Result<u16, String> {
    let req = http::Request::builder()
        ...
        .method("POST")
        .uri(format!(
            "https://{SERVED_NAME}/yadgar.project.v1.ProjectService/Probe"
        ))
...
async fn a_certificate_that_cannot_be_read_refuses_the_boot() {
    let p = pki(SERVED_NAME);
    let key = TempPem::with(&p.key_pem);
    let missing = std::env::temp_dir().join("yadgar-project-no-such-cert-6a19f4.pem");
...
async fn a_private_key_that_cannot_be_read_refuses_the_boot() {
    let p = pki(SERVED_NAME);
    let cert = TempPem::with(&p.cert_pem);
    let missing = std::env::temp_dir().join("yadgar-project-no-such-key-2f80bd.pem");
