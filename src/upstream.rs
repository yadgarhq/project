//! Connecting to `project-db`, this module's twin.
//!
...
//! module-specific wiring on top: `project-db`'s env-configured host, port and
...
/// `<PREFIX>_TLS_CLIENT_CERT_FILE` and `<PREFIX>_TLS_CLIENT_KEY_FILE`. This service has one upstream, `PROJECT_DB`; the gateway
...
/// Connect to `project-db` and return a load-balanced [`Channel`].
///
/// `project-db`'s Service is headless (D23), so this goes through `yadgar_dial` —
...
mod tests {
    ...
    const SENTINEL_CLIENT_CERT: &str = "/etc/yadgar/aardvark-9f3c/project-caller.pem";
    ...
    fn asking_for_tls_without_a_ca_bundle_is_an_error() {
        for vars in [
            vec![("PROJECT_DB_TLS_ENABLED", "1")],
            vec![
                ("PROJECT_DB_TLS_ENABLED", "1"),
                ("PROJECT_DB_TLS_CA_FILE", ""),
            ],
            vec![
                ("PROJECT_DB_TLS_ENABLED", "1"),
                ("PROJECT_DB_TLS_CA_FILE", "   "),
    ...
    const UNRESOLVABLE: &str = "project-db-no-such-host-4b7e02.invalid";
    ...
    ///
    /// **AN ABSENT `project-db` IS NO LONGER A FAILED DIAL, and this case is
    ...
    #[test]
    fn an_absent_project_db_is_published_as_a_gauge() {
        let (emitted, _channel) = dial_under_a_recorder(UNRESOLVABLE);
        assert!(
            gauge_for(&emitted, UNRESOLVABLE, 1.0),
            "a project-db that never resolved must be published as a gauge an \
    ...
    #[test]
    fn a_project_db_that_resolves_publishes_the_same_gauge_at_zero() {
        let (emitted, _channel) = dial_under_a_recorder(RESOLVABLE);
        assert!(
            gauge_for(&emitted, RESOLVABLE, 0.0),
            "a resolvable project-db must still publish the series, at zero: \
             {emitted:?}"
        );
    }
